...
*/
template <class Real>
bool Choleskydcmp(NRMat<Real>& L_, const Int4& ibegin, const Int4& iend)
{
...
template <class Real>
Real Determinant(const SymMat<Real>& S)
{
	const Int4 dim = S.size();
	NRMat<Real> mat(dim,dim);
	for(Int4 i=0; i<dim; i++)
	{
		for(Int4 j=0; j<=i; j++)
		{
			mat[i][j] = S(i,j);
	...
	Choleskydcmp(mat, 0, dim);

	Real ans = 1.0;
	for(Int4 i=0; i<dim; i++) ans *= mat[i][i];
	return ans * ans;
}

#endif // JAMA_CHOLESKY_H
